...
初始化阶段依次读取配置和元数据、建立投影与 MPI 分解、分配 `block_type`、读取化学初值和静态气象场。第一步积分前，模式在气象场准备完成后写出 `start_time` 的初始快照。
...
所有 domain 都完成当前根时间步后，如果结束时刻命中 `output_timedelta`，模式将各 domain 的气象场插值并诊断到该结束时刻，再写出瞬时浓度和气象场。
