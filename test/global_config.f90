...
contains
   ...
   subroutine test_config(error)
      ...
      if (.not. allocated(error)) call check(error, config%proj_id == 0 .and. config%is_global .and. &
                           abs(config%delta - 1._fp) <= epsilon(1._fp) .and. abs(config%xorgs(1) + 180._fp) <= epsilon(1._fp), &
                    'global MOCK projection is not a one-degree global grid')
      if (.not. allocated(error)) call check(error, trim(config%mete_source) == 'mock' .and. len_trim(config%ic_file) == 0 .and. &
                           len_trim(config%bc_file) == 0 .and. len_trim(config%emis_file) == 0, &
