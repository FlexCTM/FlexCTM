...
contains
   ...
   subroutine test_fallbacks(error)
      ...
      call block_data%init(proc%bridges(1), proc%tiles(1), 3, 1, &
                           chemistry, meteorology, 1)
      call block_data%mesh%init(-180._fp, -90._fp, 45._fp, &
                                block_data%nx, block_data%ny, 0, projection_definition)

      call initialize_chemistry(proc, proc%domains(1), proc%tiles(1), block_data, &
                                '/tmp/flexctm-missing-initial-input.nc')
      call update_emission(proc, proc%domains(1), proc%tiles(1), block_data, &
                           '/tmp/flexctm-missing-emission-input.nc', 1)

      ozone_index = chemistry%idx('O3')
      carbon_monoxide_index = chemistry%idx('CO')
      call check(error, all(block_data%chem3d == 0._fp), &
                 'missing initial file did not produce a zero field')
      if (.not. allocated(error)) &
         call check(error, maxval(block_data%emis3d(:, :, 1, carbon_monoxide_index)) > 0._fp .and. &
                           all(block_data%emis3d(:, :, 2:, carbon_monoxide_index) == 0._fp), &
                    'fallback emissions are not confined to the surface layer')
      if (.not. allocated(error)) &
         call check(error, all(block_data%emis3d(:, :, :, ozone_index) == 0._fp), &
