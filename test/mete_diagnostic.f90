...
contains
   ...
   subroutine setup(block_data, proc)
      ...
      call block_data%mesh%init(0._fp, 0._fp, 1._fp, block_data%nx, block_data%ny, &
   ...
   subroutine test_diagnostics(error)
      ...
      do k = 1, block_data%nz
         call check(error, all(block_data%volume(:, :, k) == &
                               block_data%mesh%area*block_data%dz(:, :, k)), &
                    'cell volume does not equal area times thickness')
         if (allocated(error)) exit
      end do
      if (.not. allocated(error)) &
         call check(error, all(ieee_is_finite(block_data%kx)) .and. &
                           all(ieee_is_finite(block_data%ky)) .and. &
                           all(ieee_is_finite(block_data%kz)), &
                    'diffusivities are not finite')
      if (.not. allocated(error)) &
         call check(error, all(block_data%kx >= 0._fp) .and. all(block_data%ky >= 0._fp) .and. &
                           all(block_data%kz >= 0._fp), 'diagnosed diffusivity is negative')
      if (.not. allocated(error)) &
         call check(error, all(ieee_is_finite(block_data%Ri)) .and. &
                           all(ieee_is_finite(block_data%ustar)) .and. &
                           all(block_data%ustar >= 0._fp), 'surface-layer diagnostics are invalid')
      if (.not. allocated(error)) &
         call check(error, all(block_data%P == pressure) .and. &
                           all(block_data%T == temperature) .and. &
                           all(block_data%rho == density), &
                    'common diagnostics modified standard fields')
