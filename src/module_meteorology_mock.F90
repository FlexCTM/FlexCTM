   ...
   subroutine generate_mock_meteorology(iblock)
      ...
      do j = 1, iblock%ny
         do i = 1, iblock%nx
            ...
            iblock%mete2d(i, j, iblock%m2d_idx%get('z0')) = 0.03_fp + 0.17_fp*cos(latitude)**2
            do k = 1, iblock%nz
               height = 500._fp*real(k, fp)
               iblock%u(i, j, k) = 5._fp + 25._fp*exp(-((abs(iblock%mesh%mlat(i, j)) - 45._fp)/15._fp)**2)
               ...
               iblock%thetav(i, j, k) = iblock%T(i, j, k)*(P00/iblock%P(i, j, k))**R_cp
            end do
         end do
      end do
   ...
   subroutine generate_mock_static(iblock)
      ...
      do j = 1, iblock%ny
         do i = 1, iblock%nx
            ...
            iblock%terrain(i, j) = 1800._fp*exp(-((longitude - 85._fp)/18._fp)**2 - ((latitude - 32._fp)/10._fp)**2) + &
               900._fp*exp(-((longitude + 70._fp)/20._fp)**2 - ((latitude + 20._fp)/12._fp)**2)
         end do
      end do
